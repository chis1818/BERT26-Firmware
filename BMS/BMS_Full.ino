...
static constexpr float   PACK_V_CHARGE_MAX  = 400.0f;  // V inverter limit, CV target
...
static FaultCode checkCellFaults() {
  ...
  if (g_state == STATE_CHARGE_ACTIVE && pack_sum > PACK_V_CHARGE_MAX) return FAULT_PACK_OV;
...
void handleChargeActive() {
  ...
  if (runningVoltTimer >= RUNNING_VOLT_MS) {
    ...
    // Charge done: any cell at max OR charger current tapered
    bool cell_full = (!isnan(g_vstats.mx) && g_vstats.mx >= CELL_V_MAX);
    bool tapered   = (g_charger_out_a > 0.0f && g_charger_out_a < CHARGER_DONE_A);
    if (cell_full || tapered) {
      ...
      Serial.printf("[BMS] Charge done (cell_full=%d, tapered=%d) -> CHARGE_DONE\n",
                    cell_full, tapered);
      return;
    }
  }
